...
export class MockVolcengineClient implements VolcengineClient {
   ...
   * 模拟提交异步字幕擦除任务
   */
  async submitEraseTask(_videoUrl: string): Promise<EraseTaskResponse> {
    // Reference parameter to satisfy strict eslint rule in all build setups
    console.log(`[MockVolcengineClient] Simulating task submission for video: ${_videoUrl}`);
    const timestamp = Date.now();
    const taskId = `amk-mock-erase-task-${timestamp}`;
  ...
  async queryEraseTask(taskId: string): Promise<EraseTaskQueryResponse> {
    const timestampPart = taskId.replace('amk-mock-erase-task-', '');
